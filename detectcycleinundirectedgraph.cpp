...
map<int,bool> ma;
bool detect_cycle(vector<int> adj[], int n,bool visited[], int x){
    if(ma[n]) return 1;
    ma[n] = 1;  
    if(!visited[n]){
        visited[n] = 1;
    for(int i=0;i<adj[n].size();i++){
        if(adj[n][i]!=x && detect_cycle(adj,adj[n][i],visited,n)) return 1;
    }
}
    ma[n] = 0;
    return 0;
...
bool solve(int A, vector<int> adj[]){
   ...
   for(int i=0;i<A;i++){
    if(!visited[i]){
        if(detect_cycle(adj,i,visited,-1)) return 1;
    }
   }
   return 0;
}
